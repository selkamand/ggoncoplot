...
To sort oncoplot clinical annotations, just specify columns of your metadata data.frame that you want to sort by using the `metadata_sort_cols` argument. Specifying multiple columns (as shown below) will do a hierarchical/stratified sort (sorts on first column, then by second column, etc).
