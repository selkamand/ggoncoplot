...
ggoncoplot <- brca_df |>
  ggoncoplot(
    ...
    options = ggoncoplot_options(
      show_genebar_labels = TRUE, plotsize_metadata_rel_height = 30, plotsize_tmb_rel_height = 10, genebar_label_nudge = 20,
    ),
    interactive = FALSE,
    verbose=FALSE
...
tsne <- express_precomputed_tsne("BRCA", interactive = FALSE)

## Combine plots with patchwork
combined_plots <- wrap_plots(ggoncoplot, tsne, ncol = 1, guides = "collect", heights = c(7, 3))

# View the interactive version with ggiraph
interactive_multiplot <- girafe(ggobj = combined_plots, height_svg = 6, width_svg = 12)
...
interactive_multiplot <- girafe_options(x = interactive_multiplot,
    opts_selection(type = "multiple", only_shiny = FALSE, css = "fill: purple; stroke:purple")
    )
